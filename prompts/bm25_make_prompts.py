...
def output_dir_for_k(top_k: int) -> Path:
    # cartella richiesta: prompts/prompts_bm25_3snip (o _{K}snip)
    return Path(__file__).parent / f"prompts_bm25_{top_k}snip"
...
def discover_rag_builders() -> Dict[str, Callable[[str, str], str]]:
    """
    Trova tutte le funzioni 'build_rag_prompt_*' nel package prompts.
...
# ------------- helpers -------------
def _join_snippets(snips: List[str], sep: str) -> str:
    return sep.join(snips) if snips else ""

def _snippets_from_topk_record(rec: Dict[str, Any], k: int) -> List[str]:
    """
    rec['topk'] = [ { 'rank': 1, 'snippet': '...' }, ... ]
    """
    arr = rec.get("topk") or []
    arr = sorted(arr, key=lambda x: x.get("rank", 10**9))
    return [it.get("snippet", "") for it in arr[:k] if isinstance(it.get("snippet", ""), str)]


# ------------- core -------------
def generate_prompts_from_bm25(
    top_k: int = 3,
    separator: str = "\n\n# --- Snippet Separator ---\n\n",
    preferred_first: Optional[str] = "build_rag_prompt_v6_3",
    force: bool = False,
    show_progress: bool = True,
) -> Dict[str, Path]:
    """
    Per ogni builder 'build_rag_prompt_*' crea i prompt per tutte le query del file
    BM25/retrieved_k{K}_samples.json e salva in 'prompts/prompts_bm25_{K}snip/{builder}.json'.
    ...
    # 2) builders
    builders = discover_rag_builders()
    if not builders:
        raise RuntimeError("Nessuna funzione 'build_rag_prompt_*' trovata nel package 'prompts'.")
    ...
    for bname in names:
        ...
        print(f"Building prompts with '{bname}' → {out_path.name}")
        builder = builders[bname]

        items: List[Dict[str, Any]] = []
        iterator = results if not show_progress else tqdm(results, desc=f"Building ({bname})", unit="query")
        ...
        for rec in iterator:
            ...
            snips = _snippets_from_topk_record(rec, top_k)
            joined = _join_snippets(snips, separator)

            try:
                prompt_str = builder(instruction, joined)
            except TypeError:
                raise TypeError(f"Il builder '{bname}' deve avere firma (instruction, snippets_block).")
        ...
        print(f"  Saved {len(items)} prompts → {out_path}")
        outputs[bname] = out_path
...
if __name__ == "__main__":
    ...
    #   python -m prompts.bm25_make_prompts
    generate_prompts_from_bm25(top_k=3, force=False, show_progress=True)
