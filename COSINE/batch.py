...
def run_cosine_retrieval(
    dataset,                              # Iterable[Dict[str, Any]] (e.g. Dataset from HF)
    ...
    """
    Esegue retrieval 'cosine' per tutte le query del dataset appartenenti a target_repos.
    Salva tre file:
      - outputs/retrieval/cosine_raw_k{top_k}.json
      - outputs/retrieval/cosine_topk_k{top_k}.jsonl
      - outputs/retrieval/cosine_topk_k{top_k}.json
    Ritorna i path e alcune stats.
    """
    METRIC = "cosine"
    ...
    print(f"[COSINE] queries selezionate: {len(data_iter)} | k={top_k}")

    raw_rows: List[Dict[str, Any]] = []
    norm_rows: List[Dict[str, Any]] = []

    for i, item in enumerate(tqdm(data_iter, desc="COSINE")):
        ...
        try:
            ...
            ret = get_retriever(
                metric=METRIC, kb_file=str(kb_file_path), kb_root=None,
                k1=bm25_k1, b=bm25_b, model_name=model_name, batch_size=batch_size, rrf_k=rrf_k
            )
